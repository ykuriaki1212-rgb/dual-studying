// ========================================
// スタディダッシュボード Google Apps Script（修正版）
// ========================================

// ========== 設定 ==========
// ※ この方法ではSPREADSHEET_IDの設定は不要です
// スプレッドシートから直接「拡張機能」→「Apps Script」で開いた場合、
// 自動的に紐付けられます
...
// ========== スプレッドシート取得 ==========
// openById()ではなく、getActiveSpreadsheet()を使用
...
function testInit() {
  try {
    ...
    console.log('');
    console.log('次のステップ:');
    console.log('1. formatSheets() を実行して書式を整える（任意）');
    console.log('2. 「デプロイ」→「新しいデプロイ」でWebアプリを公開');
    ...
    console.error('- スプレッドシートから「拡張機能」→「Apps Script」で開きましたか？');
    console.error('- 初回実行時に認証を許可しましたか？');
  }
}
